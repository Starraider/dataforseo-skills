...
compatibility: "Requires filesystem write access and the official DataForSEO MCP server with ONPAGE and DATAFORSEO_LABS enabled. Python 3 is recommended for deterministic validation and normalization."
---
...
2. Require one absolute HTTP(S) URL. When Python 3 is available, run `python3 <this-skill-directory>/scripts/metadata_support.py filename --url <url> --date <local-date>` to validate it and obtain the normalized domain and filename. Otherwise apply the equivalent input and path contract directly; do not stop solely because Python is unavailable. Ask and wait when the URL is absent or invalid.
...
4. Call both OnPage tools for the validated request URL. Use only exposed arguments. Do not enable JavaScript initially. Audit current metadata from Instant Pages, then classify the content as either `structured_main_topic` or `projection_degraded_text`, using the helper when available or the documented response paths and filters directly. Derive topic, intent, geography, seeds, and suggestions only from qualifying Content Parsing evidence.
...
8. Normalize and rank results with `scripts/metadata_support.py` when Python 3 is available; otherwise perform the documented normalization, merge, validation, and ranking rules directly. Preserve null versus zero, merged seed provenance, metric conflicts, exclusions, coverage, and the derived `opportunity_proxy` formula in either mode.
