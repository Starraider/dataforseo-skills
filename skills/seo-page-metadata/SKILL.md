...
4. Call both OnPage tools for the validated request URL. Use only exposed arguments. Do not enable JavaScript initially. Audit current metadata from Instant Pages, but derive topic, intent, geography, seeds, and suggestions only from qualifying Content Parsing evidence.
5. Continue only when the content contract supports five distinct non-brand seeds. Otherwise stop before keyword calls, explain the evidence shortfall, and ask before a JavaScript retry or other billable recovery.
